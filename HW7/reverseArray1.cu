...
__global__ void cu_reverseArray(int arr_in[]) {
    ...
    int temp = arr_in[x];
    arr_in[x] = arr_in[ARRAY_SIZE - x - 1];
    arr_in[ARRAY_SIZE - x - 1] = temp;
...
//    cu_reverseArray <<<dimgrid, dimblock>>> (arr_in_d, arr_out_d);
    cu_reverseArray <<<dimgrid, dimblock>>>(arr_in_d);
    cudaEventRecord(stop);

    // transfer results back to host
//    result = cudaMemcpy (arr_out, arr_out_d, sizeof(int) * ARRAY_SIZE, cudaMemcpyDeviceToHost);
    result = cudaMemcpy(arr_in, arr_in_d, sizeof(int) * ARRAY_SIZE, cudaMemcpyDeviceToHost);
    if (result != cudaSuccess) {
